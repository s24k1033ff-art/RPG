...
'''
html = html.replace('this.x += this.vx * dt; this.y += this.vy * dt;', wall_collision.strip())

# 2. ボスのバリア無効化と攻撃追加（既存コードを一切削除せず、Enemy.update の先頭に強制処理を追加）
boss_hook = '''
        if (this.type === 'boss') {
            this.isInvincible = false;
            if (typeof bossCrystals !== 'undefined') bossCrystals.forEach(c => c.active = false);
            if (this.attackCooldown <= 0 && Math.random() < 0.05) {
                const angle = Math.atan2(player.y - this.y, player.x - this.x);
                enemyProjectiles.push(new Projectile(this.x, this.y, angle, 150, this.atk, '#f43f5e', 8, true));
                this.attackCooldown = 1.5;
            }
        }
'''
html = html.replace('update(playerX, playerY) {', 'update(playerX, playerY) {\n' + boss_hook)

# 3. 中ボスステータス
boss_stats = "if(type === 'mini_boss') { this.maxHp = 150; this.radius = 20; this.color = '#eab308'; this.speed = 70; this.xp = 150; this.gold = 100; this.atk = 15; this.dropItem = '鉄くず'; } else "
html = html.replace("if(type === 'slime') {", boss_stats + "if(type === 'slime') {")
...
force_portals = '''
        if (typeof currentSceneId !== 'undefined' && typeof player !== 'undefined') {
...
print('Ultimate flawless patch applied without any deletion.')
