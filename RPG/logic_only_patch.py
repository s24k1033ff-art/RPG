...
# 5. 強制ポータル（updateループ内に差し込む）
# マップのポータルデータが壊れていても確実に移動できるようにする
force_portals = '''
...
'''
html = re.sub(r'(function update\(\) \{)', r'\1\n' + force_portals, html)
...
print('Success: logic-only patch applied without touching sceneData.')
