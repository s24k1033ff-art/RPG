...
# 1. 街とギルドのポータルを安全に修正
# 正規表現で、'city': { から portals: [ ... ] までを対象とするが、タイルデータを壊さないように
# portals: \[[\s\S]*?\] だけを置換する。このとき、直前に 'city': ... または 'guild': ... があるブロックに限定するため、
# 先に split して処理する
parts = html.split("'guild': {")
if len(parts) == 2:
    # guild側のポータル修正
    parts[1] = re.sub(r'portals: \[[\s\S]*?\]', r"portals: [\n                    { c: 40, r: 28, dest: 'city', sc: 40, sr: 12 }\n                ]", parts[1], count=1)
    
    # city側のポータル修正 (parts[0] の中で 'city': { の後にある portals を置換)
    subparts = parts[0].split("'city': {")
    if len(subparts) == 2:
        subparts[1] = re.sub(r'portals: \[[\s\S]*?\]', r"portals: [\n                    { c: 40, r: 10, dest: 'guild', sc: 40, sr: 25 },\n                    { c: 40, r: 5, dest: 'dungeon_f1', sc: 40, sr: 74 }\n                ]", subparts[1], count=1)
        parts[0] = "'city': {".join(subparts)
    
    html = "'guild': {".join(parts)


# 2. 敵の壁抜け防止 (Enemy.update内)
wall_collision = '''
