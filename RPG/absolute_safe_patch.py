...
# 1. 街とギルドのポータルの安全な修正
# 元の壊れたポータル文字列をそのまま置換する。
# split を使って、前半（city周辺）と後半（guild周辺）で分けて置換する。
bad_portal = "portals: [\n                    { c: 10, r: 14, dest: 'city', sc: 40, sr: 16 }\n                ]"
city_portal = "portals: [\n                    { c: 40, r: 10, dest: 'guild', sc: 40, sr: 25 },\n                    { c: 40, r: 5, dest: 'dungeon_f1', sc: 40, sr: 74 }\n                ]"
guild_portal = "portals: [\n                    { c: 40, r: 28, dest: 'city', sc: 40, sr: 12 }\n                ]"

# city の portals は html 全体の中で最初の bad_portal と仮定する。
html = html.replace(bad_portal, city_portal, 1)
# guild の portals は次に見つかる bad_portal と仮定する。
html = html.replace(bad_portal, guild_portal, 1)
...
for d in dungeons:
    ...
    html = re.sub(pattern, add_portal, html, count=1)


with open('index.html', 'w', encoding='utf-8') as f:
    f.write(html)

print('Success: absolutely safe patch applied.')
